               ...
               ((car) 			; gets the first element of a
                ...
                (let ((r (car (normal-eval-impl (nth exp 1)))))
                  (if (and (list? r)
                      ...
                      r)))
               ((cdr) ; gets the second element of a cons cell and
		      ; forces it if needed
...
(define (apply-normal proc args)
  ...
  ;; to the function in place of the params and then evaluate it.
      (cond ((procedure? proc) (apply proc (map normal-eval-impl args)))
            ((and (list? proc) 
                  (eq? (car proc) 'closure))
                 (normal-eval-impl (substitute (nth proc 1) 
                                               (map (partial apply cons)
                                                    (zip (nth proc 2) args)))))
            (else (normal-eval-impl (cons proc args)))))
...
(define (substitute form substitutions)
  (let ((replacement (assoc form substitutions)))
    (cond (replacement (cdr replacement))
          ((list? form)
           (case (car form)
                 ((quote) form)
                 ((lambda) `(lambda ,(nth form 1) ,(substitute (nth form 2) substitutions)))
                 ((define) `(define ,(nth form 1) ,(substitute (nth form 2) substitutions)))
                 (else (map (lambda (x) (substitute x substitutions)) form))))
          (else form))))
